...
Keep the response structured with clear headers, using bolding, bullet points, and clean spacing. Do not include introductory or concluding conversational chit-chat, just provide the structured explanation directly.`;
  ...
  // 3. API route: Generate AI Blog Image (Future use)
  app.post("/api/ai/generate-blog-image", async (req, res) => {
    ...
    try {
      ...
      console.warn("Image generation fallback active:", err.message || err);
      // Fallback with a premium styled vector or medical image representation
      res.json({ 
        imageUrl: `https://images.unsplash.com/photo-1576091160550-2173dba999ef?auto=format&fit=crop&w=600&q=80`,
        note: "Server is in fallback mode. Provided a premium medical/nursing stock illustration representation.",
        error: err.message
      });
    }
  });

  // 4. API route: Generate advanced question rationales (Future use)
  app.post("/api/ai/generate-rationale", async (req, res) => {
    const { question, options, correctAnswerIndex } = req.body;
    if (!question || !options) {
      return res.status(400).json({ error: "Missing 'question' or 'options' array." });
    }
    ...
    try {
      ...
      const prompt = `You are a Senior Indian Nursing Officer & Premier Tutor. Create a detailed, professional, and visually structured rationale/explanation for this Nursing Officer exam practice question:
      
QUESTION: ${question}
OPTIONS:
${options.map((opt: string, idx: number) => `${idx + 1}. ${opt}`).join("\n")}
CORRECT ANSWER INDEX: ${correctAnswerIndex !== undefined ? correctAnswerIndex + 1 : "Not specified"}

Please break down the rationale into four logical components:
1. 🔑 MASTER CLINICAL CONCEPT: Detail the physiological or anatomical basis in full professional depth.
2. 📖 WHY THIS IS THE RIGHT CHOICE: Explicitly connect standard Indian Nursing Council (INC) protocols to the correct response.
3. 💡 HIGH-YIELD MNEMONIC: Provide a simple memory aid to retain this.
4. ⚠️ DISTRACTOR ANALYTICS: Quickly state why other options are incorrect or represent common clinical exam traps.
`;
      ...
      if (!text) {
        throw new Error("No explanation text generated.");
      }

      res.json({ rationale: text });
    } catch (err: any) {
      console.warn("Rationale generation fallback active:", err.message || err);
      res.json({ 
        rationale: `🔑 MASTER CLINICAL CONCEPT: General medical-surgical or pediatric nursing protocol standard.\n\n📖 WHY THIS IS RIGHT: Option aligns perfectly with INC and Central Govt recruitment guidelines (AIIMS NORCET).\n\n💡 CLINICAL KEY: Always prioritize patient airway safety and hemodynamic status before elective interventions.\n\n*(Server is running in high-yield local fallback mode)*`,
        error: err.message
      });
    }
  });
